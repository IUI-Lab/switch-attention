...
class SupConLoss(nn.Module):
	...
	def forward(self, features, labels=None, mask=None):
		device = (torch.device('cuda:1') if features.is_cuda else torch.device('cpu'))
...
def focal_loss(alpha = None,
	...
	Args:
		alpha (Sequence, optional): Weights for each class. Will be converted to a Tensor if not None. Defaults to None.
		gamma (float, optional): A constant, as described in the paper. Defaults to 0.
		reduction (str, optional): 'mean', 'sum' or 'none'. Defaults to 'mean'.
		ignore_index (int, optional): class label to ignore. Defaults to -100.
